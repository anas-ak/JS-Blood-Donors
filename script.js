...
function callIfChildren(e) {
    ...
    if (e.target !== this) setRecipients(e);
}
...
async function setRecipients(e) {
    e.target.classList.add("highlight");
    lastCalled = e;

    const donor = e.target.innerText;
    for(let item of BLOOD_TYPES[donor]) {
        const recipient_index = Object.keys(BLOOD_TYPES).indexOf(item);
        const height = 50 + 50 * Math.floor(recipient_index / 2);
        const blood_height = 125 - 25 * Math.floor(recipient_index / 2);
        blood_bag.style.height = `${blood_height}px`;
        center_via.style.height = `${height}px`;

        await timeout(100);
        blood_vias[recipient_index].style.width = '100%';
        blood_types[recipient_index].classList.add("highlightText");
    }
}